...
- **Audit logging system**: Create `AuditLog` model with fields (action, actorId/User ref, targetType/collection name, targetId/document ID, before/after snapshots, timestamp, ipAddress, userAgent), TTL index on timestamp for 30-day auto-deletion, middleware hook to automatically log all database modifications in API routes
...
|----|-------|--------|--------|----------|
| [001](docs/tickets/001-recipe-database.md) | Recipe and ingredient database management (admin delete/clean operations with email notifications) | Open | wayfinder:research | — |
| [002](docs/tickets/002-user-promotion.md) | User promotion to administrator (role model migration, enforce 1+ admin/super-admin exist) | Open | wayfinder:research | — |
| [003](docs/tickets/003-feature-flags.md) | Feature flag system exploration and selection (Unleash/LaunchDarkit vs MongoDB-based) | Open | wayfinder:research | — |
...
| [005](docs/tickets/005-user-activity.md) | User activity monitoring (centralized audit logging with 30-day TTL, middleware hook) | Open | wayfinder:task | — |
| [006](docs/tickets/006-audit-logging.md) | Centralized audit logging system implementation (AuditLog model, TTL indexing, automatic logging middleware) | Open | wayfinder:task | — |
...
Frontier tickets (open, unblocked): 001, 002, 003, 004, 005, 006, 007
...
**Questions addressed**:
- **Q1-Q7**: Scope boundaries, user model location, database architecture, feature flags deferred to research, activity logging from scratch, ban details, super-admin enforcement
- **Q8**: Email system confirmed at `server/services/emailService.js` with nodemailer + Ethereal sandbox; alert on failure required
- **Q9**: Multi-role model: separate Roles collection not embedded field
- **Q10-Q14**: All recommendations accepted for audit logs, admin routes, feature flags, ban middleware, API-only implementation
